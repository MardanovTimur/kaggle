...
if __name__ == '__main__':
    ...
    train_df['Title'] = train_df['Name'].apply(lambda X: re.search('[A-Z]{1}[a-z]+\.', X).group(0))
    test_df['Title'] = test_df['Name'].apply(lambda X: re.search('[A-Z]{1}[a-z]+\.', X).group(0))
    ...
    pop(('Name', 'Cabin'), test_df, train_df)
    ...
    title_class = dict([(title, val) for title, val in zip(title_classes, np.linspace(0, 1, len(title_classes)))])
    ...
    passenger_id, X_train = train_df['PassengerId'].to_numpy(), \
        train_df.loc[:, 'Pclass':].to_numpy()

    passenger_id_test, X_test = test_df['PassengerId'].to_numpy(), \
        test_df.loc[:, 'Pclass':].to_numpy()
    ...
    submission = pd.concat([pd.Series(passenger_id_test, name='PassengerId'),
                            submission], axis=1)
    submission.to_csv(DATA_PATH / 'final_submission_v3.csv', index=False)
